...
static int32_t OV5640Core_WritePREG(struct OV5640_WREG const *pTb,int32_t len)
{
	...
	{
		if (0 == pTb[i].mask) {  
			ov5640_i2c_write(ov5640_client->addr,pTb[i].addr,pTb[i].data,10,BYTE_LEN);
		} else {   
			ov5640_i2c_read_byte(ov5640_client->addr,pTb[i].addr, &regv);
			regv &= pTb[i].mask;
			regv |= (pTb[i].data & (~pTb[i].mask));
			ov5640_i2c_write(ov5640_client->addr, pTb[i].addr, regv, 10 ,BYTE_LEN);
		}
	}
	return ret;
}
...
static void camera_sw_power_onoff(int v)
{
	...
	if (v == 0) {
    	...
    	if (v == 0)
            {
		pr_err("camera_sw_power_onoff: down\n");
		ov5640_i2c_write(ov5640_client->addr, 0x4202, 0x0f, 10,BYTE_LEN); //power down
		mdelay(50);
	      }
	else
        {
		pr_err("camera_sw_power_onoff: on\n");
		ov5640_i2c_write(ov5640_client->addr, 0x4202, 0x00, 10,BYTE_LEN); //power on
	 }
}
...
static int ov5640_video_config(void)
{
	...
	rc = ov5640_i2c_write(ov5640_client->addr, 0x3000, 0x0020, 1,BYTE_LEN);
	if (rc < 0)
	{
	...
	rc = ov5640_i2c_write(ov5640_client->addr, 0x3000,0x0000, 1,BYTE_LEN);
	if (rc < 0)
	{
		pr_err("%s: failed, rc=%d!\n", __func__, rc);
		return rc;
	...
	// release focus status		    
	 rc = ov5640_i2c_write(ov5640_client->addr, 0x3023, 0x0001, 1,BYTE_LEN);
	 if (rc < 0)
	{
		pr_err("%s: failed, rc=%d!\n", __func__, rc);
		return rc;
	...
	rc = ov5640_i2c_write(ov5640_client->addr, 0x3022, 0x0008,1, BYTE_LEN);
	if (rc < 0)
	{
		pr_err("%s: failed, rc=%d!\n",__func__, rc);        
		return rc;
	...
	rc = ov5640_i2c_write(ov5640_client->addr, 0x3022, 0x0012,1, BYTE_LEN);
	if (rc < 0)
	{
		return rc;
	} 
...
static int ov5640_setting(enum msm_s_reg_update rupdate,enum msm_s_setting rt)
{
	...
	case S_REG_INIT:
		...
		rc = ov5640_i2c_write(ov5640_client->addr, 0x3103, 0x11, 10,BYTE_LEN);
		rc = ov5640_i2c_write(ov5640_client->addr, 0x3008, 0x82, 10,BYTE_LEN);
		msleep(5);
		...
		if (tmp != 0x21) {
			rc = ov5640_i2c_write(ov5640_client->addr, 0x4740, 0x21, 10,BYTE_LEN);
			msleep(10);
			rc =ov5640_i2c_read_byte(ov5640_client->addr, 0x4740, &tmp);			   
...
static long ov5640_set_effect(int mode, int effect)
{
	...
	switch (effect) 
	{
		case CAMERA_EFFECT_OFF: 
		{
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5583, 0x0040, 1,BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5584, 0x001e, 1, BYTE_LEN);//0x28 kenxu update @20120518
            if (rc < 0)
            {
               return rc;
            }
    ...
    case CAMERA_EFFECT_MONO: 
		{
			pr_err("--CAMERA-- %s ...CAMERA_EFFECT_OFF\n",__func__);
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5583, 0x0080, 1,BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5584, 0x0080, 1,BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
	...
	case CAMERA_EFFECT_SEPIA: 
		{
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5584, 0x00a0, 1,BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
	...
	case CAMERA_EFFECT_NEGATIVE:
		{
		    ...
		    rc = ov5640_i2c_write(ov5640_client->addr, 0x5584, 0x0028, 1, BYTE_LEN);
	            if (rc < 0)
	            {
	               return rc;
	            }
...
static int ov5640_set_brightness(int8_t brightness)
{
	...
	case CAMERA_BRIGHTNESS_0:
		pr_err("--CAMERA--CAMERA_BRIGHTNESS_LV0\n");
        {
                    ...
                    rc = ov5640_i2c_write(ov5640_client->addr, 0x5587, 0x0030, 1,BYTE_LEN);
                    if (rc < 0)
                    {
                       return rc;
                    }
	...
	case CAMERA_BRIGHTNESS_1:
		pr_err("--CAMERA--CAMERA_BRIGHTNESS_LV1\n");	
        {
                    ...
                    rc = ov5640_i2c_write(ov5640_client->addr, 0x5587, 0x0020, 1, BYTE_LEN);
                    if (rc < 0)
                    {
                       return rc;
                    }
	...
	case CAMERA_BRIGHTNESS_2:
		pr_err("--CAMERA--CAMERA_BRIGHTNESS_LV2\n");
        {
                    ...
                    rc = ov5640_i2c_write(ov5640_client->addr, 0x5587, 0x0010, 1, BYTE_LEN);
                    if (rc < 0)
                    {
                       return rc;
                    }
	...
	case CAMERA_BRIGHTNESS_3:
		pr_err("--CAMERA--CAMERA_BRIGHTNESS_LV3\n");
        {
                    ...
                    rc = ov5640_i2c_write(ov5640_client->addr, 0x5587, 0x0000, 1,BYTE_LEN);
                    if (rc < 0)
                    {
                       return rc;
                    }
	...
	if (effect_value == CAMERA_EFFECT_OFF)
	{
		...
		case CAMERA_CONTRAST_0:
			pr_err("--CAMERA--CAMERA_CONTRAST_LV0\n");
	     {
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5586, 0x0010, 1,BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }            
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5585, 0x0010, 1,BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }            
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5588, 0x0001, 1,BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }            
		  	...
		  	//rc = OV5640Core_WritePREG(ov5640_contrast_lv1_tbl);
	  {
       ...
       if (rc < 0)
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5586, 0x0018, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }            
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5585, 0x0018, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }            
			...
			pr_err("--CAMERA--CAMERA_CONTRAST_LV2\n");
	  {
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5586, 0x0020, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }            
                 rc = ov5640_i2c_write(ov5640_client->addr, 0x5585, 0x0000, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }            
	 ...
	 {  
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5586, 0x0024, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }            
		...
		{   
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5586, 0x002c, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }            
            rc = ov5640_i2c_write(ov5640_client->addr, 0x5585, 0x001c, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }        
...
static int ov5640_set_sharpness(int sharpness)
{
	...
	if (effect_value == CAMERA_EFFECT_OFF) 
	{
		...
		case CAMERA_SHARPNESS_0:
            ...
            {
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5308 ,0x0065, 1, BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        }
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5302  ,0x0018, 1, BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        }
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5303 ,0x0000,  1,BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        }
		...
		case CAMERA_SHARPNESS_1:
			pr_err("--CAMERA--CAMERA_SHARPNESS_LV1\n");			
            {
                        ...
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5302  ,0x0010,1, BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        }
		...
		case CAMERA_SHARPNESS_2:
            ...
            {
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5308 ,0x0025, 1,BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        }
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5302 ,0x0030, 1,BYTE_LEN); //0x10 //kenxu update @20120517 for ourdoor sharpness
                        if (rc < 0)
                        {
                           return rc;
                        }
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5303 ,0x0000, 1,BYTE_LEN); //0x08 //kenxu update @20120517 for ourdoor sharpness
                        if (rc < 0)
                        {
                           return rc;
                        }
		...
		case CAMERA_SHARPNESS_3:
			pr_err("--CAMERA--CAMERA_SHARPNESS_LV3\n");			
            {
                        ...
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5302 ,0x0008, 1, BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        }
		...
		case CAMERA_SHARPNESS_4:
			pr_err("--CAMERA--CAMERA_SHARPNESS_LV4\n");			
            {
                        ...
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5302 ,0x0002, 1,BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        }
	...
	{ 
		...
		case CAMERA_SATURATION_0:
			...
			//rc = OV5640Core_WritePREG(ov5640_saturation_lv0_tbl);
            {
                        ...
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5583, 0x0020, 1, BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        } 
                             
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5584, 0x0010, 1,BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        }
		...
		case CAMERA_SATURATION_1:
			...
			//rc = OV5640Core_WritePREG(ov5640_saturation_lv1_tbl);
            {
                        ...
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5583, 0x0038, 1, BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        } 
                             
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5584, 0x0020, 1,BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        }            
		...
		case CAMERA_SATURATION_3:
			...
			//rc = OV5640Core_WritePREG(ov5640_saturation_lv3_tbl);
            {
                        ...
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5583, 0x0050, 1,BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        } 
                             
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5584, 0x0038, 1,BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        }
		...
		case CAMERA_SATURATION_4:
			...
			//rc = OV5640Core_WritePREG(ov5640_saturation_default_lv4_tbl);
            {
                        ...
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5583, 0x0060, 1,BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        }
                                
                        rc = ov5640_i2c_write(ov5640_client->addr, 0x5584, 0x0048, 1, BYTE_LEN);
                        if (rc < 0)
                        {
                           return rc;
                        }
...
static long ov5640_set_antibanding(int antibanding)
{
	...
	case CAMERA_ANTIBANDING_SET_60HZ:		
	{
	    pr_err("--CAMERA--CAMERA_ANTIBANDING_60HZ\n");
            ...
            tmp_reg |= 0x0080;
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3C01, tmp_reg, 1,BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3C00, 0x0000, 1,BYTE_LEN);
            if (rc < 0)
            {
                return rc;
            }
	...
	case CAMERA_ANTIBANDING_SET_50HZ:
	{
	    pr_err("--CAMERA--CAMERA_ANTIBANDING_50HZ\n");        	  
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3C00, 0x0004, 1,BYTE_LEN);
            if (rc < 0)
            {
                return rc;
            }
...
static int ov5640_set_wb_oem(uint8_t param)
{
	...
	case CAMERA_WB_MODE_AWB:
	 ...
	 {
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3406, 0x0000, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
	...
	{
        	  rc = ov5640_i2c_write(ov5640_client->addr, 0x3406 ,0x0001, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3400 ,0x0006, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3401 ,0x001c, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3402 ,0x0004, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3403 ,0x0000, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3404 ,0x0005, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3405 ,0x0020, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
		...
		pr_err("--CAMERA--CAMERA_WB_MODE_INCANDESCENT\n");
	{
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3400 ,0x0004, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3401 ,0x0010, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3404 ,0x0008, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3405 ,0x0040, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
		...
		pr_err("--CAMERA--CAMERA_WB_MODE_FLUORESCENT\n");
	{
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3400 ,0x0005, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3401 ,0x0048, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3404 ,0x0007, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3405 ,0x00c0, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
		...
		pr_err("--CAMERA--CAMERA_WB_MODE_CLOUDY\n");  
	{
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3404 ,0x0004, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3405 ,0x00d3, 1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
...
static int32_t ov5640_set_iso(int8_t iso_val)
{
    ...
    switch (iso_val)
    {
        case CAMERA_ISO_SET_AUTO:
        {
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3A18 ,0x0000,1, BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
            #if 1 // 主观测试版本 2011-06-16 ken
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3A19 ,0x00f8, 1, BYTE_LEN);
            #else
        ...
        case CAMERA_ISO_SET_100:
        {
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3A19 ,0x0020, 1,BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
        ...
        case CAMERA_ISO_SET_200:
        {
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3A19 ,0x0040, 1,BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
        ...
        case CAMERA_ISO_SET_400:
        {
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3A19 ,0x0080, 1,BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
        ...
        case CAMERA_ISO_SET_800:
        {
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3A19 ,0x00fc, 1,BYTE_LEN);
            if (rc < 0)
            {
               return rc;
            }
 ...
 */
   //Use Trig Auto Focus command to start auto focus	
	...
	if(ov5640_TouchAF_x >= 0 && ov5640_TouchAF_y >= 0) {
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3024, (int)ov5640_TouchAF_x, 10,BYTE_LEN);
            if (rc < 0)
            {
                pr_err("%s: failed, rc=%d!\n", __func__, rc);
                goto done;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3025, (int)ov5640_TouchAF_y, 10,BYTE_LEN);
            if (rc < 0)
            {
                pr_err("%s: failed, rc=%d!\n",__func__, rc);        
                goto done;
            ...
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3023, 0x0001, 10,BYTE_LEN);
            if (rc < 0)
            {
                pr_err("%s: failed, rc=%d!\n", __func__, rc);
                goto done;
            }
            rc = ov5640_i2c_write(ov5640_client->addr, 0x3022, 0x0081, 10,BYTE_LEN);
            if (rc < 0)
            {
                pr_err("%s: failed, rc=%d!\n",__func__, rc);        
                goto done;
        ...
        rc = ov5640_i2c_write(ov5640_client->addr, 0x3022, 0x0003, 1, BYTE_LEN);
        if (rc < 0)
        {
            pr_err("%s: failed, rc=%d!\n",__func__, rc);        
            goto done;
