...
static ssize_t mtd_read(struct file *file, char __user *buf, size_t count,loff_t *ppos)
{
	...
	while (count) {
		...
		default:
			...
			if (  strcmp(mtd->name, "appsbl")  == 0
			  || strcmp(mtd->name, "mibib")  == 0 || strcmp(mtd->name, "qcsbl")   == 0
			  || strcmp(mtd->name, "oemsbl1")== 0 || strcmp(mtd->name, "oemsbl2") == 0 
			   || strcmp(mtd->name, "APPSBL")  == 0 
			  || strcmp(mtd->name, "MIBIB")  == 0 || strcmp(mtd->name, "QCSBL")   == 0 
			  || strcmp(mtd->name, "OEMSBL1")== 0 || strcmp(mtd->name, "OEMSBL2") == 0 ){ 
			    /* on 7x27a-ics, "amss" is no longer read as modem spec */
				if(mtd->read_modem != NULL){
					ret = mtd->read_modem(mtd, *ppos, len, &retlen, kbuf);
				}
				else
					pr_info("%s: mtd->read_modem is NULL\n", __func__);
	...
	while (count) {
		...
		default:
			...
			if (  strcmp(mtd->name, "appsbl")  == 0
			  ...
			  || strcmp(mtd->name, "OEMSBL1")== 0 || strcmp(mtd->name, "OEMSBL2") == 0 ){ 
			    /* on 7x27a-ics, "amss" is no longer read as modem spec */
				if(mtd->write_modem != NULL){
					ret = mtd->write_modem(mtd, *ppos, len, &retlen, kbuf);
				}
				else
					pr_info("%s: mtd->write_modem is NULL\n", __func__);
			} else {
#endif
