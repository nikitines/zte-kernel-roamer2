...
//rxz modify for read flash info, start
#include <linux/proc_fs.h>
#include <mach/zte_memlog.h>
//rxz modify for read flash info,end
#include "msm_nand.h"
...
#ifdef CONFIG_ZTE_NAND_GOTA
static unsigned msm_nand_set_modem_cfg0(struct mtd_info *mtd)
{
	unsigned cfg0;
	uint8_t wide_bus;
	uint32_t mtd_writesize;
	...
	cfg0 = (((mtd_writesize >> 9)-1) << 6) /* 4/8 cw/pg for 2/4k */
...
static unsigned msm_nand_set_modem_cfg1(struct mtd_info *mtd)
{
	unsigned cfg1;
	uint8_t wide_bus;
	uint32_t mtd_writesize;
...
static uint32_t msm_nand_set_modem_ecc_buff(void)
{
	return 0x1ff;
}
#endif
	...
	while (page_count-- > 0) {
...
#ifdef CONFIG_ZTE_NAND_GOTA
			if(ops->mode == MTD_OOB_MODEM){ 
				dma_buffer->data.cfg0 = (msm_nand_set_modem_cfg0(mtd) & ~(7U << 6))
					| (((cwperpage-1) - start_sector) << 6);
				dma_buffer->data.cfg1 = msm_nand_set_modem_cfg1(mtd);
			}else{
#endif
				dma_buffer->data.cfg0 =
				(chip->CFG0 & ~(7U << 6))
...
#ifdef CONFIG_ZTE_NAND_GOTA
				if(ops->mode == MTD_OOB_MODEM) 
					dma_buffer->data.ecccfg = msm_nand_set_modem_ecc_buff();
				else
#endif
		...
		if (pageerr) {
			...
			(dma_buffer->data.result[n].buffer_status & 0x8)) {
					/* not thread safe */
#ifdef CONFIG_ZTE_NAND_GOTA
					if(ops->mode == MTD_OOB_MODEM){ /* ZTE_NAND_GOTA */
						pageerr = 0;
						break;
					} else {
#endif
		...
		if (ops->mode != MTD_OOB_RAW) {
			...
			if(ops->mode == MTD_OOB_MODEM){ 
				if (start_sector == (cwperpage - 1)) {
					dma_buffer->data.cfg0 = (msm_nand_set_modem_cfg0(mtd) &
								~(7U << 6));
				} else {
					dma_buffer->data.cfg0 = (msm_nand_set_modem_cfg0(mtd) &
					~(7U << 6))
...
#ifdef CONFIG_ZTE_NAND_GOTA
int msm_nand_read_modem(struct mtd_info *mtd, loff_t from, size_t len, size_t *retlen, u_char *buf)
{
	int ret;
	struct mtd_oob_ops ops;
...
#ifdef CONFIG_ZTE_NAND_GOTA
int msm_nand_write_modem(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const u_char *buf)
{
	int ret;
	struct mtd_oob_ops ops;
...
#ifdef CONFIG_ZTE_NAND_GOTA
	mtd->read_modem = msm_nand_read_modem;
	mtd->write_modem = msm_nand_write_modem;
#endif
        ...
        char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	/*rxz, modify for read flash info,end*/
	else if((msm_nand_global->flash_id[0]==0xad)&&(msm_nand_global->flash_id[1]==0xb3))
	{
		if(msm_nand_global->sdrem_length	==	1)
			len = sprintf(page, "%s\n","Hynix 8G NAND");
		else
...
no_dual_nand_ctlr_support:
	...
	init_memory_proc();		////rxz, modify for read flash info
...
static int __devexit msm_nand_remove(struct platform_device *pdev)
{
	...
	deinit_memory_proc();		//rxz, modify for read flash info
	return 0;
}
